...
```{python}
#| label: fig-puissance-pompe
#| fig-cap: "Lecture énergétique du fonctionnement d'une pompe. À gauche : le point de fonctionnement est l'intersection de la courbe de la pompe et de la courbe du réseau. À droite : le rendement à ce point conditionne directement la puissance électrique consommée. Un point de fonctionnement hors de la plage de rendement optimal pénalise fortement la facture énergétique."
import numpy as np
import matplotlib.pyplot as plt
...
ax1.plot(Qf, Hf, "o", color=C_ACCENT, ms=10, zorder=5)
ax1.annotate(f"Point de fonctionnement\n$Q_f$={Qf:.2f} m³/s, $H_f$={Hf:.1f} m",
             xy=(Qf, Hf), xytext=(Qf+0.06, Hf+9), fontsize=8.5, color=C_ACCENT,
             arrowprops=dict(arrowstyle="->", color=C_ACCENT))
ax1.set_xlabel("Débit $Q$ (m³/s)"); ax1.set_ylabel("Hauteur manométrique $H$ (m)")
ax1.set_title("Point de fonctionnement", fontsize=10.5)
ax1.legend(fontsize=8.5, loc="lower center")
ax1.set_xlim(0, 0.5); ax1.set_ylim(0, 50); ax1.grid(alpha=0.25)

ax2.plot(Q, eta*100, color=C_PIEZO, lw=2.4)
ax2.plot(Qf, etaf*100, "o", color=C_ACCENT, ms=10, zorder=5)
ax2.axvline(Qf, color=C_ACCENT, ls=":", lw=1)
ax2.annotate(f"$\\eta$ = {etaf*100:.0f} % au point\nde fonctionnement",
             xy=(Qf, etaf*100), xytext=(Qf-0.20, etaf*100-26), fontsize=8.5,
             color=C_ACCENT, arrowprops=dict(arrowstyle="->", color=C_ACCENT))
ax2.axvspan(0.24, 0.36, color=C_PIEZO, alpha=0.12)
ax2.text(0.30, 12, "Plage de\nrendement\noptimal", ha="center", fontsize=8, color=C_PIEZO)
ax2.set_xlabel("Débit $Q$ (m³/s)"); ax2.set_ylabel("Rendement $\\eta$ (%)")
ax2.set_title("Rendement énergétique", fontsize=10.5)
ax2.set_xlim(0, 0.5); ax2.set_ylim(0, 95); ax2.grid(alpha=0.25)
fig.suptitle("Lecture énergétique du fonctionnement d'une pompe", fontsize=11.5)
fig.tight_layout()
plt.show()
```
